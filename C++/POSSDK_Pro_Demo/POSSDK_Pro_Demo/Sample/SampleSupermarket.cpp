...
int Sample_Supermarket(int handle, char *errorStatus)
{
	...
	nRet = mPrintText(handle, "**************************************\n", "");
	mFeedLines(handle, 1);
	mBarCodePrint(handle, BarCode_Code128, "1234567890123", strlen("1234567890123"), "x=-2");
	nRet = mPrintText(handle, "谢谢惠顾，欢迎再次光临", "x=-2");
...
int Sample_Supermarket_En(int handle, char *errorStatus)
{
	...
	nRet = mPrintText(handle, "Total Sales:		1			Total Return: -0\n", "");
	...
	mBarCodePrint(handle, BarCode_Code128, "1234567890123", strlen("1234567890123"), "x=-2");
	nRet = mPrintText(handle, "Thank you for shopping at M&S.", "Align=1|Bold=1");
	nRet = mPaperCut(handle, 0, 0);
